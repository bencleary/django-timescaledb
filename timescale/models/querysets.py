...
class TimescaleQuerySet(models.QuerySet):
    ...
    def time_bucket(self, fields: Iterable[str], time_field: str, interval: str, annotations: Dict = None):
        """
        Wraps the TimescaleDB time_bucket function into a queryset method.
        """
        if annotations:
            return self.values(*fields, bucket=TimeBucket(time_field, interval)).order_by('-bucket').annotate(**annotations)
        return self.values(*fields, bucket=TimeBucket(time_field, interval)).order_by('-bucket')
